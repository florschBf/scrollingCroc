...
class EncounterController:
        ...
        if self.game_over:
            ...
            if encounter[0] == 'message':
                ...
                for obstacle in encounter:
                    if obstacle != "encounter":
                        amount = obstacle[0]
                        obstace_type = obstacle[1]
                        pattern = obstacle[2]
                        y_start = obstacle[3]
                        for x in range(amount):
                            self.generate_obstacle(obstace_type, pattern, y_start)
        else:
            # level over
            ...
            if len(self.scene.active_sprites.sprites()) == 0:
                ...
                self.scene.ui_handler.create_message_to_player('Gewonnen!!!'
                                                               ...
                                                               , 'Solange die Finger und der Computer durchhalten.'
                                                               , 'Space gedrückt halten geht auch...')

    def generate_obstacle(self, obstacle_type, pattern, y_start='random'):
        """
        Method to spawn obstacles to the scene
        :param obstacle_type: Currently available obstacles as String are 'obstacle' and 'enemy'
    ...
    def update(self):
        ...
        if not self.endless:
            if (sprite_count == 0 and current_time - self.board_clear >= self.empty_delay) or (
                    current_time - self.last_timestamp > self.delay):
